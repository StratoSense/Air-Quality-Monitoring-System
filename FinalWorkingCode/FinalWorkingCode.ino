...
unsigned long sendInterval = 5000;  // Default 5 seconds
const unsigned long minInterval = 5000;   // 5 sec minimum
const unsigned long maxInterval = 600000; // 10 min maximum
...
void sendToBlynkBulk(float temp, float hum, int pm1, int pm25, int pm10, int aqi, String cat, float mq7, float mq135);
void readMQ7();
void readMQ135();
float readRs_MQ135();
...
// ---------- Dynamic Send Interval (Blynk Input V20) ----------
BLYNK_WRITE(V20) {
  int userInput = param.asInt(); // value from app in seconds

  if (userInput < (minInterval / 1000)) userInput = minInterval / 1000;
  if (userInput > (maxInterval / 1000)) userInput = maxInterval / 1000;

  sendInterval = (unsigned long)userInput * 1000UL;
  Serial.printf("⚙️ Send interval updated to %lu seconds\n", sendInterval / 1000);

  // Optional: Show confirmation in app
  Blynk.virtualWrite(V21, String("Interval: ") + String(sendInterval / 1000) + "s");
}
...
void sendSMS(String message) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String postData = "To=" + String(toNumber) + "&From=" + String(smsNumber) + "&Body=" + message;
...
// ---------- Blynk ----------
// ---------- Blynk ----------
void sendToBlynkBulk(float temp, float hum, int pm1, int pm25, int pm10,
                     int aqi, String cat, float mq7, float mq135) {
  ...
  // 🔥 Automatically detect warm-up period (first 10 minutes)
  bool warmingUp = (millis() < 600000); // 600000 ms = 10 min
  ...
  // Always send these base values
  url += "&V0=" + String(temp, 1);
  url += "&V1=" + String(hum, 1);
  url += "&V4=" + String(pm1);
  url += "&V5=" + String(pm25);
  url += "&V6=" + String(pm10);
  ...
  if (warmingUp) {
    Serial.println("🔥 MQ7 & MQ135 warming up — skipping gas sensor data.");
  } else {
    // CO & Gas text encoding
    String coText = "CO:%20" + String(mq7, 1) + "%20ppm%20" + getMQ7Status(mq7);
    String gasText = "Gas:%20" + String(mq135, 0) + "%20ppm%20" + getMQ135Status(mq135);
  ...
  for (int attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (code == 200) {
      ...
      Serial.printf("⚠️ Send attempt %d failed (code %d)\n", attempt, code);
      delay(500);
    }
  }

  Serial.printf("❌ Final Blynk Error %d\n", code);
}
...
void readMQ7() {
  ...
  if (mq7_ppm < 1) mq7_ppm = 1;
  mq7_status = getMQ7Status(mq7_ppm);

  // --- Auto baseline micro-adjust (prevents "stuck" readings around 4.2 ppm) ---
  static unsigned long lastAdjust = 0;
  if (millis() - lastAdjust > 60000) { // every 60s
    if (mq7_ppm > 3 && mq7_ppm < 5) {
      R0_MQ7 = R0_MQ7 * 0.999 + Rs * 0.001; // tiny drift correction
    }
    lastAdjust = millis();
  }
}
...
float getGasPPM_MQ135(float Rs) {
  if (baselineR0_MQ135 <= 0.0 || Rs <= 0.0) return 1.0;  // avoid 0 divide
  float ratio = Rs / baselineR0_MQ135;
  if (ratio < 0.01) ratio = 0.01; // clamp minimum
  float ppm = GAS_A * pow(ratio, GAS_B);
  if (!isfinite(ppm) || ppm < 1) ppm = 1; // ensure valid positive number
  return ppm;
}
...
void readMQ135() {
  ...
  else ppm_smoothed_MQ135 = (SMOOTH_ALPHA * raw) + ((1.0 - SMOOTH_ALPHA) * ppm_smoothed_MQ135);

  // force refresh when change is very small
  if (fabs(raw - ppm_smoothed_MQ135) < 0.5) {
    ppm_smoothed_MQ135 = raw;
  }

  mq135_ppm = ppm_smoothed_MQ135;
  if (mq135_ppm < 1) mq135_ppm = 1;
  mq135_status = getMQ135Status(mq135_ppm);

  // --- Auto baseline micro-adjust (prevents drift or "stuck" low values) ---
  static unsigned long lastAdjust = 0;
  if (millis() - lastAdjust > 60000) { // every 60 seconds
    if (mq135_ppm > 3 && mq135_ppm < 8) {  // only adjust in clean-air range
      baselineR0_MQ135 = baselineR0_MQ135 * 0.999 + Rs * 0.001;
    }
    lastAdjust = millis();
  }
}
...
void updateOLED() {
  ...
  if (!warmupDone) {
    ...
    unsigned long remain = (WARMUP_DURATION > elapsed) ? (WARMUP_DURATION - elapsed) : 0;
...
void loop() {
 ...
 if (current_AQI > AQI_Threshold) {
  String msg = "High AQI: " + String(current_AQI) + " (" + current_AQI_category + ")";
  Blynk.logEvent("aqi_alert", msg.c_str());
}

if (mq7_ppm > MQ7_ALERT_THRESHOLD) {
  String msg = "High CO: " + String(mq7_ppm, 1) + " ppm";
  Blynk.logEvent("co_alert", msg.c_str());
}

if (mq135_ppm > MQ135_ALERT_THRESHOLD) {
  String msg = "High Gas: " + String(mq135_ppm, 0) + " ppm";
  Blynk.logEvent("general_gas_alert", msg.c_str());
}
